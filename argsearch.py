...
class ARGS:
    ...
    def generate(self, input_ids, weight=0., topk=1, max_new_token=128, method="greedy", temperature=0.7, chunk_size=5, debug=False):
...
if __name__ == "__main__":
    ...
    LLM_PATH = "../../CKPT/argsearch/llama-7b-sft-float32"
    RM_PATH = "../../CKPT/argsearch/llama-7b-rm-float32"
    LLM_PATH = "./checkpoints/Llama-2-7b-SFT"
    RM_PATH = "./checkpoints/Llama-2-7b-SFT-RM"
    
    searcher = ARGS(llm_path=LLM_PATH, rm_path=RM_PATH, llm_dev="cuda:0", rm_dev="cuda:1")
    print(searcher.RM.config.pad_token_id)
    text = ""
    input_ids = searcher.get_input_ids(text)
    print("input:", searcher.tokens_to_text(input_ids))
    # args-greedy decoding with weight=1.0
    output_tokens = searcher.generate(input_ids, topk=10, weight=1.0, method="greedy", max_new_token=64)
